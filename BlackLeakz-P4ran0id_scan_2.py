...
"color: white;")
    ...
    def runx(self):
            targetx = self.terminalcmd_inp.text()
            self.terminal.append(targetx)
            self.p = QProcess()
            self.p.readyReadStandardOutput.connect(self.handle_stdout)
            self.p.start("tracert ", [targetx])
    ...
    def send_terminal(self):
        ...
        if cmd == "!t":
                self.terminal.append("Console<>> HELP_DOCUMENTATION ")
                self.terminalcmd_inp.returnPressed.connect(self.runx)
           #     targetx = cmd.replace("!t " , "tracert ")
